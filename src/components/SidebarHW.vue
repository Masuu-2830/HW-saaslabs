        ...
        <div id="nav-links-container" class="pd-r-10">
          <nav class="nav nav-sidebar tx-13" id="labels-nav">
            ...
            <RouterLink
                ...
                :key="index"
                :to="{name: 'type', params:{type: section.type, mailboxId: getMailboxID}}"
            >
                <p
                    style="cursor: pointer"
                    id="mentions-label"
                    class="nav-link hw-label-badge mg-b-0"
                    :class="getRouteParamType == section.type && 'active'"
                >
                    <span v-html="section.icon"></span>
                    <span>{{section.name}}</span>
                    <span class="badge text-primary hw-count" id="mentions-count">
                        {{mailbox.stats[section.stats] > 0 || (typeof mailbox.stats[section.stats]) === 'string' ? mailbox.stats[section.stats] == 0 ? '' : mailbox.stats[section.stats] : ''}}
                    </span>
                </p>
            </RouterLink> 
