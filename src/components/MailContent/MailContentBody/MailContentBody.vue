  ...
  >
    ...
    <div v-if="Object.keys(thread).length !== 0" class="emails-wrapper">
      <div v-for="(item, index) in thread.data.items" :key="index">
        <mail-content-single-mail
          v-if="item.type == 'email' || item.type == 'custom'"
          :isCollapsed="lastMailId !== item.data.id"
          :subject="thread.data.displaySubject"
          :item="item"
  ...
  computed: {
    lastMailId() {
      if (Object.keys(this.thread).length !== 0) {
        let mails = this.thread.data.items
          .filter((item) => item.type == "email" || item.type == "custom")
          .map((item) => item.data);
