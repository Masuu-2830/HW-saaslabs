<template>
  ...
  <div
    ...
    class="df-settings-body pd-l-10 pd-r-15"
  >
    <div id="contactDetailsWrapper" style="">
      <div class="pd-t-20">
        ...
        >
          ...
          >
          <div class="d-flex flex-column w-100" id="chatUser_edit_contact_div">
            <label
      ...
      >
        <div class="collapse-body shadow-sm bg-white rounded bd">
          <div
            ...
            href="#chatUser_recent-pages_details"
            role="button"
            aria-expanded="false"
            aria-controls="chatUser_recent-pages_details"
            style="height: 50px; cursor: pointer"
          >
            <div class="col-10">
              <p class="pd-l-13 pd-t-15 tx-bold">Recent Pages</p>
            </div>
          ...
          <div
            id="chatUser_recent-pages_details"
            class="chatUser_collapsible pd-b-1 pd-x-8 collapse"
            style="overflow-wrap: break-word; line-height: 1.5"
          >
                      ...
                      >
                        ...
                        <div class="timeline-body">
                          <a
                            :href="page.page_url"
                            target="_blank"
                            class="btn btn-link recentPagePopover pd-0"
                            role="button"
                            tabindex="0"
                            data-container="body"
                            data-toggle="popover"
                            data-placement="top"
                            data-content="Helpwise Sample Help Center"
                            style="
                              outline: none;
                              cursor: pointer;
                              color: black;
            ...
            <div class="row pd-x-10">
              <button
                v-if="shownPages.length > 5"
                @click.stop.prevent="lessPages"
                type="button"
                class="btn btn-link pd-5 col-5"
                id="showLessRecentPages"
              >
                Show Less
              </button>
              <button
                v-if="shownPages.length < contact.data.recentPages.length"
                @click.stop.prevent="morePages"
                type="button"
                class="btn btn-link pd-5 col-5 offset-7"
                id="showMoreRecentPages"
              >
                Show More
              </button>
            </div>
              ...
              <div v-for="activity in activities" :key="activity.id">
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      <span
                        ...
                        aria-expanded="false"
                        :aria-controls="'activity-inside-div-' + activity.id"
                        class=""
                        ><i
                          class="fas fa-caret-down"
                          id=""
                  ...
                  <div
                    ...
                    style=""
                  >
                    <p style="color: #999da0ad; font-size: 13px" class="mg-t-5">
                      Start date<br /><span
                    ...
                    <p style="color: #999da0ad; font-size: 13px" class="mg-t-5">
                      End date<br /><span
                        style="color: #4f5d6b; font-size: 13px"
                        >{{
                          activity.body.end_date | moment("MMM D, YYYY hh:mm a")
                        }}</span
                      >
                    </p>
                    <p style="color: #999da0ad; font-size: 13px" class="mg-t-5">
                    ...
                    <div class="form-group d-flex align-items-center">
                      <div class="custom-control custom-radio">
                        <input
                          ...
                          value="Call"
                        />
                        <label class="custom-control-label" for="activity-call"
                          >Call</label
                        >
                      </div>
                    ...
                    <div class="modal-body pd-20 pd-sm-30">
                      <div id="all-groups-container">
                        <a
                          ...
                          class="dropdown-item btn list-group-item map-group"
...
export default {
  ...
  data() {
    return {
      ...
      // contact: {},
      id:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.id !== undefined
          ? this.thread.data.contact.id
          : "",
      firstname:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.firstname !== undefined
          ? this.thread.data.contact.firstname
          : "",
      lastname:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.lastname !== undefined
          ? this.thread.data.contact.lastname
          : "",
      company:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.company !== undefined
          ? this.thread.data.contact.company
          : "",
      job:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.jobTitle !== undefined
          ? this.thread.data.contact.jobTitle
          : "",
      activities:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.activities !== undefined
          ? this.thread.data.contact.activities
          : [],
      groups:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.groups !== undefined
          ? this.thread.data.contact.groups
          : [],
      notes:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.notes !== undefined
          ? this.thread.data.contact.notes
          : [],
      email:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.emails !== undefined &&
      ...
      phones:
        Object.keys(this.thread).length !== 0 &&
        this.thread.data.contact.phones !== undefined &&
  ...
  beforeMount() {
    ...
    if (
      ...
      this.contact.data.recentPages.length > 0
    ) {
      this.shownPages = this.contact.data.recentPages.slice(0, 5);
    }
  },
  methods: {
    date(date) {
    ...
    morePages() {
      if (this.contact.data.recentPages.length > this.shownPages.length) {
        this.shownPages = this.contact.data.recentPages.slice(
          0,
          this.shownPages.length + 5
        );
      }
    },
    lessPages() {
      if (this.shownPages.length % 5 == 0) {
        this.shownPages = this.contact.data.recentPages.slice(
          0,
          this.shownPages.length - 5
        );
        ...
        this.shownPages = this.contact.data.recentPages.slice(
          0,
          this.shownPages.length - (this.shownPages.length % 5)
        );
      }
    },
    openInt() {
