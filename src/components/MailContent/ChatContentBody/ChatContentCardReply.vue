...
export default {
  ...
  data() {
    ...
    return {
      ...
      config: {
        ...
        toolbarButtons: ["attachCardReply", "scheduleReply", "savedReplyCCR", "hcArticleCCR", "clear"],
      },
    };
  },
  watch: {
  ...
  created() {
    ...
    bus.$on("modal.hcArticleInsert.click", function (data, type) {
      ...
      if(type == 'chatCardReply') {
        let editorInstance1 = vueThis.editorInstance;
        editorInstance1.html.insert(vueThis.getArticleCard(data));
      }
    });
    ...
    bus.$on("modal.savedReplyInsert.click", function (id, type) {
      console.log(type, id);
      if(type == 'chatCardReply') {
      console.log(vueThis.editorInstance, this.editorInstance, self.editorInstance);
      // let editorInstance1 = vueThis.editorInstance;
      // console.log(editorInstance1);
      ...
      fetch(
        ...
        .then((response) => {
          if (response.status == "success") {
            vueThis.editorInstance.html.insert(response.data.savedReply.content);
  ...
  methods: {
    ...
    prepareFroalaButtons() {
      ...
      FroalaEditor.RegisterCommand("savedReplyCCR", {
        // Button title.
        ...
        callback: function () {
          ...
          bus.$emit("savedReply", 'chatCardReply');
      ...
      FroalaEditor.RegisterCommand("hcArticleCCR", {
        ...
        callback: function () {
          ...
          console.log("----");
          bus.$emit("hcArticles", 'chatCardReply');
